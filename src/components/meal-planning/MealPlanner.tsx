...
const MealPlanner = memo((_props: MealPlannerProps) => {
  ...
  const getMealsForDay = useCallback(
    (dayOfWeek: number) => {
      if (!displayMealPlan?.meals) return [];
      return displayMealPlan.meals.filter(
  ...
  // Only show skeleton if loading AND no data exists yet (data is undefined)
  // If we have data (even if empty), show the component immediately
  if (isLoading && mealPlan === undefined) {
                      ...
                      >
                        ...
                        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto custom-scrollbar">
                          <DialogHeader>
                            ...
                            <DialogDescription>
                              Select a recipe from your favourites to add to{" "}
                              {day.dayName}&apos;s{" "}
