...
// In local dev: Next.js serves both frontend and API on same origin (port 3000)
 ...
 * 
 * - If NEXT_PUBLIC_API_URL is set: Use it (for production or custom backend)
 * - If NEXT_PUBLIC_API_URL is empty: Use relative paths with current origin (dynamic port support)
 * 
 * This ensures:
 * - Localhost works on any port (3000, etc.) automatically
