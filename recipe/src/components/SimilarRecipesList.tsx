 ...
 */
const SimilarRecipesList = memo(({ similarRecipes, className = "" }: SimilarRecipesListProps) => {
  ...
  return (
    <Card className={`bg-gradient-to-br from-blue-900/30 to-indigo-900/30 border-blue-500/30 p-4 sm:p-6 ${className}`}>
      <div className="flex items-start gap-3 mb-4">
        <div className="p-2 bg-blue-500/20 rounded-lg flex-shrink-0">
          <UtensilsCrossed className="h-5 w-5 text-blue-400" />
        </div>
        <div className="flex-1">
          <h3 className="text-lg sm:text-xl font-semibold text-white mb-1">Similar Recipes</h3>
          <p className="text-sm text-gray-400">Explore related recipes you might enjoy</p>
        </div>
      </div>
        ...
        {similarRecipes.map((similarRecipe) => {
          ...
          return (
            ...
            >
              {/* Recipe Image */}
              {imageUrl && (
                <div className="flex-shrink-0 w-20 h-20 sm:w-24 sm:h-24 rounded-lg overflow-hidden border border-blue-500/20">
                  <img
                    src={imageUrl}
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {similarRecipe.sourceUrl && (
                    <Button
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        window.open(similarRecipe.sourceUrl, '_blank', 'noopener,noreferrer');
                      }}
                    >
                  ...
                  {similarRecipe.imageType && (
                    <Badge variant="outline" className="bg-slate-700/50 border-slate-600 text-gray-300 text-xs">
