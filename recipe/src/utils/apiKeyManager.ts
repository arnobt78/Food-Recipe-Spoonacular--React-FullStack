...
function getAllApiKeys(): ApiKeyConfig[] {
  ...
  const primaryKey = import.meta.env.VITE_SPOONACULAR_API_KEY;
  if (primaryKey) {
    keys.push({
      key: primaryKey,
      used: 0,
  ...
  // Get additional API keys from environment (VITE_SPOONACULAR_API_KEY_2, _3, etc.)
  let index = 2;
  // eslint-disable-next-line no-constant-condition
  while (true) {
    const envKey = import.meta.env[`VITE_SPOONACULAR_API_KEY_${index}`];
    if (!envKey) break;
