...
> **Last verified: 2026-08-13** (web-search-verified against provider docs and independent trackers on this date). Free tiers change frequently — providers add credit-card requirements, cut rate limits, or deprecate models with little notice. Treat every row below as "true as of last verification," not a permanent guarantee. Re-check a provider's own pricing/docs page before depending on it in production.
>
...
Rows marked **🔑 have key** indicate this project (Recipe Guide / recipe-spoonacular) already documents that provider in `.env.example`. Do not commit real keys.
...
|---|---|---|---|
| **Google AI Studio (Gemini)** 🔑 | **Yes** — genuinely indefinite, no card, no expiration | `gemini-2.5-flash`, `gemini-2.5-flash-lite` (Pro tier is now paid-only as of an April 2026 change) | OpenAI-compatible endpoint confirmed working: `https://generativelanguage.googleapis.com/v1beta/openai/`. ~1,500 req/day, 1M TPM, 15 RPM on Flash. Free-tier prompts may be used to improve Google's products (paid tier / Vertex AI opts out of this). |
...
| **OpenRouter** 🔑 | **Yes** — no card for `:free` models | 28+ models with a `:free` ID suffix, e.g. `openai/gpt-oss-20b:free`, `deepseek/deepseek-chat-v3-0324:free`, `meta-llama/llama-3.3-70b-instruct:free`, `qwen/qwen3-coder-480b:free` | Single API routes to many upstream providers. Free tier: 20 req/min, 50 req/day (raised to 1,000/day if you ever add $10 of credit — still optional). The exact free-model list changes often; check `https://openrouter.ai/models?max_price=0` before depending on a specific ID. |
...
| **Cloudflare Workers AI** | **Yes** — no card, starts immediately on any free Cloudflare account | 50+ models incl. Llama, Mistral, Gemma, DeepSeek, Qwen, Whisper (speech), SDXL (image) | 10,000 free "Neurons" per day. Requires an API token *and* Cloudflare account ID. **Not wired in this repo** (no account ID in `.env.example`). |
...
**Important finding for this repo:** `llama-3.1-70b-versatile` (previously hardcoded in `app/api/[...path]/route.ts`) shut down on 2025-01-24. Remaining Llama IDs on Groq shut down 2026-08-16. The live chain is in `lib/ai/providers.ts`: `openai/gpt-oss-20b` → `qwen/qwen3.6-27b` → `openai/gpt-oss-120b`. Never add Llama IDs back.
...
This repository (Recipe Guide / recipe-spoonacular) implements the pattern at `lib/ai/` (REQ-0010 / CR-0002):

- **`types.ts`** — `ChatMessage` / `ChatResult` discriminated union (Layer 2's contract).
- **`providers.ts`** — Layer 1 registry: Groq, Gemini, OpenRouter (`:free` only), Hugging Face. Each entry is `{ envKeys, baseUrl, models[] }`.
- **`client.ts`** — one generic OpenAI-compatible `POST .../chat/completions` (Layer 2) plus 429/402/4xx skip rules.
- **`index.ts`** — outer orchestration (`completeChat`): walks `AI_PROVIDERS` in order, skips unconfigured providers, returns the first success or the last failure.
- **`parse-json.ts`** — shared JSON extraction for fenced markdown / raw objects / arrays.

Catch-all routes in `app/api/[...path]/route.ts` call `completeChat()` for `/api/ai/search`, `/api/ai/recommendations`, `/api/ai/analysis`, `/api/ai/modifications`, and weather-AI query generation. They do **not** keep per-route model IDs.

This repo does **not** wire Cloudflare Workers AI, Cerebras, NVIDIA NIM, GitHub Models, Mistral, or Cohere — those need env keys that are not in `.env.example`. Add a registry row only after the key exists.

Priority used here (recipe JSON, existing keys): **Groq → Gemini Flash/Flash-Lite → OpenRouter `:free` → Hugging Face router**.
